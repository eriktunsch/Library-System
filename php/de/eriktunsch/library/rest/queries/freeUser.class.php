    ...
    public function execute($_data)
    {
        if ($this->User->isAdmin()) {
            if (
                ...
                $stmt = $this->db->prepare("UPDATE users SET disabled=0 WHERE id=?");
                $stmt->bind_param("s", $_data["id"]);
                $stmt->execute();

                $this->generateExecutionTime();
                return $this->ResponseGenerator->GenerateResponse($this->Responses->ownResponse("success", "Die Ausleihung durch den Nutzer wurde freigegeben!"), array(), $this->execution_time);
            } else {
                $this->generateExecutionTime();
