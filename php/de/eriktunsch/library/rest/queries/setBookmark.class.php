    ...
    public function execute($_data)
    {
        if ($this->User->isAdmin()) {
            if (
                ...
                $stmt = $this->db->prepare("INSERT INTO bookmarks (`user`, book) VALUES (?, ?)");
                $stmt->bind_param("ss", $userID, $_data["isbn"]);
                $stmt->execute();

                $this->generateExecutionTime();
                return $this->ResponseGenerator->GenerateResponse($this->Responses->ownResponse("success", "Das Buch wurde zu deiner Merkliste hinzugefügt!"), array(), $this->execution_time);
            } else {
                $this->generateExecutionTime();
