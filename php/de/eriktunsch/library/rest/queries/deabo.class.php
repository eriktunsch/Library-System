...
class deabo
{
    ...
    public function execute($_data)
    {
        ...
        $stmt = $this->db->prepare("UPDATE users SET newsletter=0 WHERE id=?");
        $stmt->bind_param("s", $userid);
        $stmt->execute();

        $this->generateExecutionTime();
        return $this->ResponseGenerator->GenerateResponse($this->Responses->ownResponse("success", "Du hast den Newsletter deabonniert!"), array(), $this->execution_time);
    }
